...
SELECT @SQL
    = @SQL + 'SELECT CONVERT(VARCHAR(MAX),COUNT(*)) + '' matches in column ''+''' + C.name + '''+'' on table '' + '''
      + SC.name + '.' + T.name + ''' [Matches for ''' + @SearchString + ''':] FROM ' + QUOTENAME ( SC.name ) + '.'
      + QUOTENAME ( T.name ) + ' WHERE ' + QUOTENAME ( C.name ) + ' LIKE N''%' + @SearchString
...
WHERE
       SYST.name IN ( 'varchar', 'nvarchar', 'text', 'ntext', 'char', 'nchar' )
ORDER BY
       T.name
     , C.name;
